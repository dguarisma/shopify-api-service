...
func TestGet(t *testing.T) {
	...
	trademarksTemp := make([]model.Trademark, 3)

	for i := 0; i < len(trademarksTemp); i++ {
		trademarksTemp[i] = model.Trademark{MakerID: items[i].ID}
	}

	err = tx.Save(&trademarksTemp).Error
	assert.NoError(t, err, "it shouldn't return error when insert in database")
	...
	t.Run("All", func(t *testing.T) {
		...
		for i, item := range response {
			if i < len(trademarksTemp) {
				assert.Equal(t, item.Trademarks[0].ID, trademarksTemp[i].ID)
				assert.Equal(t, item.Trademarks[0].Status, trademarksTemp[i].Status)
				assert.Equal(t, item.Trademarks[0].Name, trademarksTemp[i].Name)
			}
...
func TestInsert(t *testing.T) {
	...
	t.Run("Inserts news and updates without delete items", func(t *testing.T) {
		...
		items2 := append(items[1:], []model.Maker{
...
func TestUpdate(t *testing.T) {
	...
	t.Run("one", func(t *testing.T) {
		item := model.Maker{Name: "example-update", Status: true}
		item.ID = itemDb.ID
		body, err := json.Marshal(item)
		assert.NoError(t, err, "json has a problem")
